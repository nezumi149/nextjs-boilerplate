    ...
    return (
       <div>
        <div style={{ height:'167px', width:'167px', backgroundColor:'#DDFFDD', position:'relative'}} onClick={() => setRotation(rotation + (props.disabled?1:0))}>
            ...
            <p style={{ display:'inline-block', position: 'absolute', top:'55%', right: '15px', webkitTransform: [{ rotate: '90deg' }]}}>
                {props.words[(1+rotation)%4]}
            </p>
            <p style={{ display:'inline-block', position: 'absolute', bottom:'0%', width:'100%', textAlign:'center',  webkitTransform: [{ rotate: '180deg' }]}}>
                {props.words[(2+rotation)%4]}
            </p>
            <p style={{ display:'inline-block', position: 'absolute', top:'45%', left: '15px',  webkitTransform: [{ rotate: '270deg' }]}}>
